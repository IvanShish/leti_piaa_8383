...
void findAllPos(const std::string& text, std::vector <Vertex>& bohr, std::vector <int>& answer);	//поиск вхождений
...
void findAllPos(const std::string& text, std::vector <Vertex>& bohr, std::vector <int>& answer) {
	...
	for (int i = 0; i < text.length(); i++) {
		...
		for (int tmp = curr; tmp != 0; tmp = getSuffLink(tmp, bohr)) {		// Обход автомата по суффикс ссылкам 
			if (bohr[tmp].flag) {		//Если при обходе встретился flag = 1
				...
				std::cout << i + 2 - bohr[tmp].deep << std::endl << std::endl;
